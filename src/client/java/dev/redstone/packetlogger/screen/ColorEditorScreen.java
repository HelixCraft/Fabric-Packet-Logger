...
import dev.redstone.packetlogger.util.DrawUtil;
...
public class ColorEditorScreen extends Screen {
    ...
    protected void init() {
        ...
        // Hex Field
        int hexWidth = 80;
        hexField = new TextFieldWidget(client.textRenderer, dialogX + 50, dialogY + dialogHeight - 35, hexWidth, 18,
                Text.empty());
        ...
        this.addDrawableChild(ButtonWidget.builder(
                ...
                button -> {
                    if (onColorSelected != null)
                        onColorSelected.accept(getARGB());
                    this.close();
                }).dimensions(dialogX + dialogWidth - 110, dialogY + dialogHeight - 35, 45, 20).build());

        this.addDrawableChild(ButtonWidget.builder(
                Text.literal("Cancel"),
                button -> this.close()).dimensions(dialogX + dialogWidth - 60, dialogY + dialogHeight - 35, 45, 20)
                .build());
    ...
    private void updateFromRGBFields() {
        ...
        try {
            ...
            float[] hsv = rgbToHsv(MathHelper.clamp(r, 0, 255), MathHelper.clamp(g, 0, 255),
    ...
    private void updateFields(boolean forceAll) {
        ...
        if (forceAll || !sField.isFocused())
            sField.setText(String.valueOf((int) (saturation * 100)));
        if (forceAll || !vField.isFocused())
            vField.setText(String.valueOf((int) (value * 100)));

        if (forceAll || !rField.isFocused())
            rField.setText(String.valueOf(rgb[0]));
        if (forceAll || !gField.isFocused())
            gField.setText(String.valueOf(rgb[1]));
        if (forceAll || !bField.isFocused())
            bField.setText(String.valueOf(rgb[2]));

        if (forceAll || !aField.isFocused())
            aField.setText(String.valueOf(a));
        if (forceAll || !hexField.isFocused())
            hexField.setText(String.format("#%08X", getARGB()));
    ...
    private int getSliderColor(int type, float t) {
        ...
        switch (type) {
            case 1:
                return hsvToRgbInt(hue, t, value); // Saturation
            case 2:
                return hsvToRgbInt(hue, saturation, t); // Value
            case 3:
                return packArgb(255, (int) (t * 255), rgb[1], rgb[2]); // R
            case 4:
                return packArgb(255, rgb[0], (int) (t * 255), rgb[2]); // G
            case 5:
                return packArgb(255, rgb[0], rgb[1], (int) (t * 255)); // B
    ...
    public boolean mouseClicked(Click click, boolean isRelease) {
        ...
        // Parameter Sliders - Map correctly: i=0->S(type1), i=1->V(type2),
        // i=2->R(type3), i=3->G(type4), i=4->B(type5), i=5->A(type6)
